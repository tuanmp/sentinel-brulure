...
def render_before(event, bands: np.ndarray):
    ...
    fig.suptitle(
        ...
        f"weather {_fmt(pre.get('weather_index'))}"
    )
    fig.tight_layout()
    return fig


def render_during(event, bands: np.ndarray, date: str):
    ndvi = compute_ndvi(bands)
    frp = None
    dets = None
    ...
    im1 = ax1.imshow(ndvi, cmap="YlGn", vmin=-0.3, vmax=0.9, extent=_extent(event))
    ax1.set_title(f"During NDVI — {date}")
    fig.colorbar(im1, ax=ax1, fraction=0.046)
    for ax in axes:
        _style_axes(ax, event)
...
def render_after(
    event, pre_bands: np.ndarray, post_bands: np.ndarray, resolution: int = 60
):
